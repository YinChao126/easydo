...
class crawling_finance:
    ...
    def parse_one_page_zichanfuzhai(self, html):
        try:
            ...
            senson_four = []
            for l in lls:
                if (l.get_text().strip()) != '流动资产' and (l.get_text().strip(
                )) != '非流动资产' and (l.get_text().strip()) != '流动负债' and (
                        l.get_text().strip()) != '非流动负债' and (
                            l.get_text().strip()) != '所有者权益':
...
# =============================================================================
            ...
            df = pd.DataFrame(
                data, index=dates, columns=features)  #转为dataframe结构
            return df
        except:
            pass
    ...
    def parse_one_page_xianjinliuliang(self, html):
        try:
            ...
            senson_four = []
            for l in lls:
                if (l.get_text().strip()) != '一、经营活动产生的现金流量' and (
                        l.get_text().strip()) != '二、投资活动产生的现金流量' and (
                            l.get_text().strip()) != '三、筹资活动产生的现金流量' and (
                                l.get_text().strip()) != '附注':
                    stock_raw_data.append(l.get_text().strip())
...
ENGINE=InnoDB DEFAULT CHARSET=utf8"""
    ...
    def df_to_mysql(self, table, df):
        connect = create_engine("mysql+pymysql://" + self.users + ":" +
                                self.passwords + "@" + self.hosts + ":3306/" +
        ...
        print("Finished insert %s data into mysql %s table." % (StockCode,
                                                                table))

    def crawling_update(self):
        date = []
        html = self.get_one_page(
            'http://money.finance.sina.com.cn/corp/go.php/vFD_BalanceSheet/stockid/'
            + self.stock_code + '/ctrl/2017/displaytype/4.phtml')
        soup = BeautifulSoup(html, 'html5lib')
...
#        self.createTables()
        ...
        for i in range(date[len(date) - 1], date[0] + 1):
            url = 'http://money.finance.sina.com.cn/corp/go.php/vFD_BalanceSheet/stockid/' + self.stock_code + '/ctrl/' + str(
                i) + '/displaytype/4.phtml'
            ...
            #             print(df)
            # =============================================================================
            zichanfuzhai = pd.concat([zichanfuzhai,
                                      df])  #两个dataframe做连接，类似数据库的union all
# =============================================================================
#         print(zichanfuzhai)
# =============================================================================
...
# Test insert into mysql
        ...
        for i in range(date[len(date) - 1], date[0] + 1):
            url = 'http://money.finance.sina.com.cn/corp/go.php/vFD_CashFlow/stockid/' + self.stock_code + '/ctrl/' + str(
                i) + '/displaytype/4.phtml'
            html = self.get_one_page(url)
            ...
            #             print(df)
            # =============================================================================
            xianjinliuliang = pd.concat([xianjinliuliang,
                                         df])  #两个dataframe做连接，类似数据库的union all
# =============================================================================
...
# cash flow insert into mysql
        ...
        for i in range(date[len(date) - 1], date[0] + 1):
            url = 'http://money.finance.sina.com.cn/corp/go.php/vFD_ProfitStatement/stockid/' + self.stock_code + '/ctrl/' + str(
                i) + '/displaytype/4.phtml'
            html = self.get_one_page(url)
...
## 在代码执行路径自动生成输入路径
#def UpdateFinnanceTable(id_list):
#    for i in id_list:
#        try:
#            cbfx = crawling_finance('',i,column_interest)
#            cbfx.crawling_update()
#        except:
#            print(i + ' cannot be update')

#############################################################################

#stock_list = ['601012','600066']
#UpdateFinnanceTable(stock_list)
...
if __name__ == '__main__':
    ...
    stock_code_num = ['600522']
    column_interest = [
        '货币资金', '应收账款', '存货', '流动资产合计', '固定资产净额', '无形资产', '资产总计', '短期借款',
        '预收款项', '流动负债合计', '长期借款', '一年内到期的非流动负债', '负债合计', '盈余公积',
        '所有者权益(或股东权益)合计'
    ]
