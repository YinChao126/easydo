...
global parameter
parameter = [
    '总资产', '净资产', '资产负债比', '流动资产', '一年内到期的长期负债', '应收账款', '预收账款', '存货', '营业收入',
    '营业成本', '营业税金及附加', '财务费用', '营业外收入', '净利润', '除非净利润', '每股收益', '经营净额', '投资净额',
    '筹资净额', '汇率影响', '现金净增加额', '期末现金余额', '流动比率', '资产周转率', '存货周转率', '溢价比', '市盈率',
    '市净率', '名义净资产收益率', '真实净资产收益率', '毛利率', '营收增长率', '除非净利润增长率', '股息率', '分红率'
]
            ...
            try:
                ...
                with open(file_name, 'r') as fh:
                    ...
                    if latest_record > current_day:
                        ...
                        cbfx = crawling_finance_table_v1_7.crawling_finance(
                            data_base_path, item)
...
def M1809_Update(cur, id_list):
    ...
    for item in id_list:
        ...
        try:
            cmd2 = "select * from cashFlow where h72 = \'" + item + "\' and h73 = \'" + str(
                datetime.now().year - 1) + "-12-31\';"
            cur.execute(cmd2)
        ...
        try:
            cmd3 = "select * from Profit where h29 = \'" + item + "\' and h30 = \'" + str(
                datetime.now().year - 1) + "-12-31\';"
            cur.execute(cmd3)
