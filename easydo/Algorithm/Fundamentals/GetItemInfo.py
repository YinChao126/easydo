...
def GetSingleItem(stock_id, year):
    ...
    #资产负债表中查询与填充
    cmd = "select * from zichanfuzhai where h79 = \'" + stock_id + "\' and h80 = \'" + str(
        year) + "-12-31\';"
    ...
    #利润表查询与填充
    cmd = "select * from Profit where h29 = \'" + stock_id + "\' and h30 = \'" + str(
        year) + "-12-31\';"
    Config.cur.execute(cmd)
    result = Config.cur.fetchall()
    result = list(result[0])
    ...
    #现金流量表查询与填充
    cmd = "select * from cashFlow where h72 = \'" + stock_id + "\' and h73 = \'" + str(
        year) + "-12-31\';"
    Config.cur.execute(cmd)
    result = Config.cur.fetchall()
    result = list(result[0])
